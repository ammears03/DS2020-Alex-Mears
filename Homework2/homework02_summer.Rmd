...
> The ames data set has 16 variables:
>
> 1.  Parcel ID - ID column with unique 10 digit ID for each column.
> 2.  Address -
> 3.  Style - Categorical column describing the frame of the housing.
> 4.  Occupancy - Categorical colunm describing the occupancy of the housing.
> 5.  Sale Date - Categorical date column detailing when the housing unit was sold.
> 6.  Sale Price - Quantitative column detailing the sale price of the housing unit.
> 7.  Multi Sale - Categorical Boolean column stating whether the sale was part of a package.
> 8.  YearBuilt -
...
> The range of Sale_Price is 20500000 - 0 = 20,500,000.
>
...
> ```
>
> The vast majority of values are smaller, with a few very large outliers. If we ignore these larger values, as well as the value 0, we can get a cleaner histogram:
>
> ```{r, echo=FALSE, message = FALSE}
> library(dplyr)
> amesadj <- filter(ames, `Sale Price` < 3057000 & `Sale Price` > 0)
> ggplot(
>   data = amesadj,
>   aes(x = `Sale Price`)
